...
SSD_crt_null_binary <- function(p_intv,
                                p_ctrl,
                                n1 = 15,
                                n2 = 30,
                                ndatasets = 1000,
    ...
    if (eta1 < 0 |
        eta0 < 0)
        stop("The probability of exceeding Bayes Factor threshold must be a positive value")
    ...
    while (ultimate_sample_sizes == FALSE) {
        ...
        if (isTRUE(data_H0$constant)) { # increase sample when the response is constant
            if (fixed == "n1") {
                if (n2 == max) {
                    return(list(
                        status  = "constant_at_max",
        ...
        while (b < (b_fract + 1)) {
            ...
            output_bain_H1 <- Map(
                bain,
                x = data_H1$estimates,
                hypothesis = "intervention=control; intervention>control",
                list(b),
                n = n_eff_H1_list,
            ...
            n_eff_H0_list <- lapply(n_eff_H0, function(x)
                c(x, x))
            output_bain_H0 <- Map(
                bain,
                x = data_H0$estimates,
            ...
            if ((prop_BF10 < eta1) & (prop_BF01 < eta0)) { # Largest absolute difference when both are less than eta
                abs_diff1 <- abs(prop_BF10 - eta1)
                abs_diff0 <- abs(prop_BF01 - eta0)
                current_eta <- ifelse(abs_diff1 > abs_diff0, prop_BF10, prop_BF01) # the futhest one
                eta <- ifelse(abs_diff1 > abs_diff0, eta1, eta0)
            } else if (conditions_met) { # the closest one to target eta
                diff1 <- prop_BF10 - eta1
                diff0 <- prop_BF01 - eta0
                current_eta <- ifelse(diff1 < diff0, prop_BF10, prop_BF01) # observed eta
                eta <- ifelse(diff1 < diff0, eta1, eta0) # target eta
            } else {
                if (prop_BF10 < eta1) {  # If proportion is lower than eta when H0 is true
                    current_eta <- prop_BF10
                    eta <- eta1
                } else if (prop_BF01 < eta0) { # If proportion is lower than eta when H1 is true
            ...
            updated_sample <- binary_search_eq(
    ...
    if (n2 < 30)
        warning(
            "The number of clusters is less than 30. This may cause problems in convergence and singularity."
        )
    invisible(final_SSD)
}
